...
use super::reporting::{ReportingConfig, ReportingPhaseHandler};
    ...
    async fn execute_workspace(
        ...
        let rep_output = if let Some(ref reporting) = pack.reporting {
            if !reporting.is_empty() {
                ...
                let mut report_config = ReportingConfig::new(
                    reporting,
                    &pack.name,
                    workspace,
                    &config.inputs,
                ...
                if let Some(proc) = proc_output.as_ref() {
                    report_config = report_config.with_processing_results(&proc.results);
                }

                if let Some(path) = config.pack_path.as_deref() {
                    report_config = report_config.with_pack_path(path);
                }

                // Pass ResultContext refs for lazy materialization
                match self.reporting.execute(report_config, progress).await
                {
