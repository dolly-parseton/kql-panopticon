...
use anyhow::Result;
use clap::Parser;
use kql_panopticon_core::tracing::{tui_channel, TuiLayer};
...
fn run_app<B: Backend>(
    ...
    app: &mut app::App,
    mut log_rx: UnboundedReceiver<kql_panopticon_core::tracing::TuiEvent>,
) -> Result<bool> {
    loop {
...
fn handle_let_statement(app: &mut app::App, command: &str) {
    ...
    let value = match kql_panopticon_core::pack::parse_input_value(raw_value) {
        Ok(v) => v,
        Err(e) => {
