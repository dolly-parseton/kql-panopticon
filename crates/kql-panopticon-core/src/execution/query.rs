//! Query executor for parallel query execution
//!
//! Handles execution of query packs - running multiple queries
//! concurrently across multiple workspaces.

use crate::client::{Client, Column, QueryResponse, Table};
use crate::error::{Error, Result};
use crate::pack::QueryPack;
...
struct StreamingJsonWriter {
    ...
    buffer_size: usize,
    table_columns: Option<Vec<Column>>,
    parse_dynamics: bool,
}
...
impl StreamingJsonWriter {
    ...
    fn add_page(&mut self, table: &Table) -> Result<()> {
        if self.table_columns.is_none() {
            return Err(Error::config(
                "Table columns not set before adding page",
            ));
        }
    ...
    async fn finalize(
        ...
        let columns = self.table_columns.as_ref().ok_or_else(|| {
            Error::config("Table columns not set")
        })?;
...
impl QueryJobBuilder {
    ...
    pub async fn execute(self, client: &Client) -> Result<Vec<QueryJobResult>> {
        let settings = self.settings.ok_or_else(|| {
            Error::config("QuerySettings not provided")
        })?;

        if self.workspaces.is_empty() {
            return Err(Error::config("No workspaces provided"));
        }

        if self.queries.is_empty() {
            return Err(Error::config("No queries provided"));
        }
...
impl QueryJob {
    ...
    async fn execute_and_save(&self, client: &Client) -> Result<JobSuccess> {
        ...
        let output_path = primary_output_path.ok_or_else(|| {
            Error::config("No export format enabled (CSV or JSON required)")
        })?;
        ...
        for attempt in 0..max_attempts {
            if attempt > 0 {
                ...
                let backoff = match &last_error {
                    Some(Error::Http { status: Some(429), .. }) => {
                        // Rate limited - use longer backoff
                        info!(
                            "Rate limited on workspace '{}'. Waiting 30 seconds before retry (attempt {}/{})",
                            self.workspace.name,
                            attempt + 1,
                            max_attempts
                        );
...
// ============================================================================
// ExecutionEngine implementation for QueryExecutor
// ============================================================================

/// Executor for query packs
///
/// Runs queries in parallel across workspaces, collecting results
/// to files and/or memory.
...
#[async_trait]
impl ExecutionEngine for QueryExecutor {
    type Config = QueryExecutorConfig;
    type Result = QueryExecutorResult;
    ...
    async fn execute(
        ...
        let start = Instant::now();
        let job_id = progress.as_ref().map(|p| p.job_id()).unwrap_or_else(uuid::Uuid::new_v4);

        let num_workspaces = workspaces.len();
        let total_steps = config.queries.len() * num_workspaces;

        // Notify progress of start
        ...
        let settings = QuerySettings {
            output_folder: config.output_dir.clone().unwrap_or_else(|| PathBuf::from("./output")),
            job_name: config.job_name.clone().unwrap_or_else(|| "query".to_string()),
            export_csv: true,
            export_json: false,
            parse_dynamics: true,
        };

        // Execute queries using QueryJobBuilder
        ...
        // Convert results to QueryExecutorResult format
        let workspace_results: Vec<WorkspaceQueryResult> = results
            .into_iter()
            .map(|r| {
                let (row_count, output_path, error) = match r.result {
                    Ok(success) => (success.row_count, Some(success.output_path), None),
        ...
        let result = QueryExecutorResult {
            job_id,
            workspace_results,
    ...
    fn validate(&self, config: &Self::Config) -> Result<()> {
        ...
        if config.queries.is_empty() {
            return Err(Error::config("No queries to execute"));
        }

        // Validate each query has content
...
pub struct QueryExecutorConfig {
    ...
    /// Timespan for queries (e.g., "P7D" for 7 days)
    pub timespan: Option<String>,
}

impl QueryExecutorConfig {
    /// Create a simple config with a single query
    ...
    /// Create a config from a query pack
    pub fn from_pack(pack: QueryPack) -> Self {
        Self {
            queries: pack.queries.iter().map(|q| q.query.clone()).collect(),
            pack: Some(pack),
            output_dir: None,
            job_name: None,
            timespan: None,
    ...
    /// Set timespan
    pub fn with_timespan(mut self, timespan: impl Into<String>) -> Self {
...
#[derive(Debug, Clone)]
pub struct QueryExecutorResult {
    /// Job ID
    pub job_id: uuid::Uuid,
    /// Results per workspace
    pub workspace_results: Vec<WorkspaceQueryResult>,
    /// Total duration in milliseconds
    pub duration_ms: u64,
    /// Output directory (if files were written)
    pub output_dir: Option<PathBuf>,
}

impl QueryExecutorResult {
    /// Get total row count across all workspaces
    pub fn total_rows(&self) -> usize {
        self.workspace_results.iter().map(|r| r.row_count).sum()
    }

    /// Get count of successful workspace queries
    ...
    /// Get count of failed workspace queries
...
/// Result for a single workspace query
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn test_config_single() {
        let config = QueryExecutorConfig::single("SecurityEvent | take 10");
        assert_eq!(config.queries.len(), 1);
        assert!(config.queries[0].contains("SecurityEvent"));
    }

    #[test]
    fn test_query_settings_default() {
        let settings = QuerySettings::default();
        assert_eq!(settings.job_name, "query");
        assert!(settings.export_csv);
        assert!(!settings.export_json);
    }
}
